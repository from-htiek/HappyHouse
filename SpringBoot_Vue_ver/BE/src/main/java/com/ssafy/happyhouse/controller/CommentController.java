...
public class CommentController {
	...
	@GetMapping("{memboardId}")
	public ResponseEntity<List<Comment>> retrieveComment(@PathVariable int memboardId) throws Exception {
		logger.debug("retrieveComment - 호출");
		return new ResponseEntity<List<Comment>>(commentService.retrieveComment(memboardId), HttpStatus.OK);
	}

	@ApiOperation(value = "댓글번호에 해당하는 댓글의 정보를 반환한다.", response = Comment.class)
